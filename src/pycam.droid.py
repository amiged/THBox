#!/usr/bin/env python3
#Time-stamp: <2017-06-12 09:34:40 hamada>
import cv2
...
#        self.ipadr  = '192.168.7.2'
        self.droidapp = 'DroidCam'
        self.ipadr  = '192.168.111.101'
        self.port  = '3389'
        self.stream = -1
        self.URL = -1
    def get_stream(self):
...
def put_text(image, text):
    ...
    cv2.putText(image, text, (5, 25), font_type, font_size, font_color)
...
if (__name__ == '__main__'):
    ...
    while True:
        ...
        state.draw_text()
        cv2.imshow('droidcam',state.image)
        cv2.setMouseCallback("droidcam", mouse_event)
        tnow = datetime.now().strftime('%Y%m%d_%H_%M_%S')
        #if True:
        if tnow != tnow0:
            basedir = '../log.droid.%d/'% (devid)
